...
SEPARATOR = "━━━━━━━━━━━━━━━━━━━━━━"
WARNING = " *WARNING:* Use only on systems you own or have permission to test. Unauthorized
# ================================================================
# START
# ================================================================
...
text = (
f"{SEPARATOR}\n"
" *CyvxBot — OSINT & Security Toolkit*\n"
f"{SEPARATOR}\n\n"
...
f" *For educational and authorized use only.*\n"
f"{SEPARATOR}"
)
...
Invalid IP address: `{ip}`", parse_mode="Mar
# Threat check
try:
threat = requests.get(f"https://api.abuseipdb.com/api/v2/check?ipAddress={ip}",
headers={"Key": "free", "Accept": "application/json"}, time
except:
abuse_score = "N/A"
abuse_score = "N/A"
...
f" Coordinates: `{d.get('latitude', 'N/A')}, {d.get('longitude', 'N/A')}`\n\n"
...
text = f" Error: `{str(e)}`"
await update.message.reply_text(text, parse_mode="Markdown", disable_web_page_preview=Tru
# ================================================================
# WHOIS
# ================================================================
async def whois_lookup(update: Update, context: ContextTypes.DEFAULT_TYPE):
if not context.args:
await update.message.reply_text("Usage: `/whois <domain>`\nExample: `/whois google.co
return
domain = context.args[0].lower().replace("https://", "").replace("http://", "").split("/"
await update.message.reply_text(f" Running WHOIS on `{domain}`...", parse_mode="Markdow
try:
...
await update.message.reply_text(f" Fetching DNS records for `{domain}`...", parse_mode=
try:
type_map = {1: "A", 2: "NS", 5: "CNAME", 15: "MX", 16: "TXT", 28: "AAAA"}
records = {}
for rtype in [1, 28, 15, 2, 16, 5]:
r = requests.get(f"https://dns.google/resolve?name={domain}&type={rtype}", d = r.json()
...
if not records:
await update.message.reply_text(f" return
No DNS records found for `{domain}`", parse_
text = (
f"{SEPARATOR}\n"
...
return
domain = context.args[0]
await update.message.reply_text(f" Finding subdomains for `{domain}`...", parse_mode="M
try:
...
if not subs:
await update.message.reply_text(f" return
No subdomains found for `{domain}`", parse_m
...
if not context.args:
await update.message.reply_text("Usage: `/github <username>`\nExample: `/github torva
return
username = context.args[0]
await update.message.reply_text(f" Investigating GitHub user `{username}`...", parse_mo
try:
r = requests.get(f"https://api.github.com/users/{username}",
headers={"User-Agent": "CyvxBot"}, timeout=10)
if r.status_code == 404:
await update.message.reply_text(f" return
GitHub user `{username}` not found.", parse_
d = r.json()
# Get repos
repos_r = requests.get(f"https://api.github.com/users/{username}/repos?per_page=5&sor
headers={"User-Agent": "CyvxBot"}, timeout=10)
...
text += f"• [{repo['name']}](https://github.com/{username}/{repo['name']}) —
text += (
...
return
username = context.args[0]
await update.message.reply_text(f" Investigating Reddit user `{username}`...", parse_mo
try:
r = requests.get(f"https://www.reddit.com/user/{username}/about.json",
headers={"User-Agent": "CyvxBot/1.0"}, timeout=10)
if r.status_code == 404:
await update.message.reply_text(f" Reddit user `{username}` not found.", parse_
return
d = r.json().get("data", {})
created = datetime.fromtimestamp(d.get("created_utc", 0), tz=timezone.utc).strftime("
cake_day = datetime.fromtimestamp(d.get("created_utc", 0), tz=timezone.utc).strftime(
text = (
f"{SEPARATOR}\n"
...
f" Moderator: `{'Yes' if d.get('is_mod') else 'No'}`\n"
...
Discord ID must be numbers only!", parse_mode="Ma
await update.message.reply_text(f" Decoding Discord ID `{user_id}`...", parse_mode="Mar
try:
...
try:
timeou
r = requests.get(f"https://discordlookup.mesalytic.moe/v1/user/{user_id}", if r.status_code == 200:
d = r.json()
...
f" Badges: `{badge_text}`\n"
f" Avatar: {'[Click here](' + avatar_url + ')' if avatar_url != 'N/A' else '`No
f" *What badges mean:*\n"
...
results.append(" Could not verify domain")
...
results.append(" Gravatar check unavailable")
text = (
f"{SEPARATOR}\n"
...
await update.message.reply_text(f" Searching `{username}` across {len(platforms)} platf
found = []
not_found = []
...
await update.message.reply_text(f" return
Could not resolve `{host}`", parse_mode="Markdow
open_ports = []
closed_count = 0
service_map = {
...
risk = " " if p in [23, 445, 3389] else " text += f" {risk} Port `{p}` — *{svc}*\n"
" if p in [21, 25, 3306] else " "
else:
...
if not context.args:
await update.message.reply_text("Usage: `/headers <url>`\nExample: `/headers google.c
return
url = context.args[0]
if not url.startswith("http"):
url = "https://" + url
await update.message.reply_text(f" Analyzing headers for `{url}`...", parse_mode="Markd
try:
...
text += f" `{h}` *MISSING*\n _{desc}_\n\n"
grade = "A" if score >= 5 else "B" if score >= 4 else "C" if score >= 3 else "D" if s
text += (
...
expiry_status = f" {days_left} days left" if days_left > 30 else f" EXPIRES S
except:
...
text = f" except Exception as e:
text = f" Error: `{str(e)}`"
`{domain}` has an *INVALID* SSL certificate!\n\n This site may be dange
await update.message.reply_text(text, parse_mode="Markdown")
# ================================================================
# ROBOTS
# ================================================================
...
if not context.args:
await update.message.reply_text("Usage: `/techstack <url>`\nExample: `/techstack goog
return
url = context.args[0]
if not url.startswith("http"):
url = "https://" + url
await update.message.reply_text(f" Detecting tech stack for `{url}`...", parse_mode="Ma
try:
...
if not context.args:
await update.message.reply_text("Usage: `/urlscan <url>`\nExample: `/urlscan google.c
return
url = context.args[0]
if not url.startswith("http"):
url = "https://" + url
await update.message.reply_text(f" Scanning `{url}` for threats...", parse_mode="Markdo
try:
domain = url.replace("https://", "").replace("http://", "").split("/")[0]
text = (
f"{SEPARATOR}\n"
f" *URL THREAT SCAN: {url}*\n"
f"{SEPARATOR}\n\n"
f" *Scan with these free tools:*\n\n"
f"• [VirusTotal](https://www.virustotal.com/gui/url/{url}) — 70+ antivirus engine
f"• [URLScan.io](https://urlscan.io/search/#{domain}) — Full page analysis\n"
f"• [Google Safe Browsing](https://transparencyreport.google.com/safe-browsing/se
...
if not context.args:
await update.message.reply_text("Usage: `/wayback <url>`\nExample: `/wayback google.c
return
url = context.args[0]
await update.message.reply_text(f" Checking archive history for `{url}`...", parse_mode
try:
...
formatted_time = f"{snap_time[:4]}-{snap_time[4:6]}-{snap_time[6:8]} {snap_time[8
text = (
f"{SEPARATOR}\n"
...
if not context.args:
await update.message.reply_text("Usage: `/crack <hash>`\nExample: `/crack 5d41402abc4
return
hash_val = context.args[0]
await update.message.reply_text(f" Attempting to crack `{hash_val}`...", parse_mode="Ma
try:
# Try MD5 crack via free API
r = requests.get(f"https://md5decrypt.net/Api/api.php?hash={hash_val}&hash_type=md5&e
result = r.text.strip()
...
# Try backup
text = (
r2 = requests.get(f"https://hashtoolkit.com/reverse-hash/?hash={hash_val}", timeo
f"{SEPARATOR}\n"
...
grade = " Weak" if strength < 2 else " Medium" if strength < 3 else " Strong" if s
text = (
f"{SEPARATOR}\n"
...
if not context.args:
await update.message.reply_text("Usage: `/cve <keyword>`\nExample: `/cve apache`", pa
return
keyword = " ".join(context.args)
await update.message.reply_text(f" Searching CVEs for `{keyword}`...", parse_mode="Mark
try:
r = requests.get(
f"https://services.nvd.nist.gov/rest/json/cves/2.0?keywordSearch={keyword}&result
timeout=15
)
data = r.json()
...
if not vulns:
await update.message.reply_text(f" return
No CVEs found for `{keyword}`", parse_mode="
text = (
f"{SEPARATOR}\n"
...
risk = " " if str(score) >= "7" else " text += f"{risk} *{cve_id}*\n"
